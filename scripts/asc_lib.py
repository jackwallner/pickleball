    ...
    if not all([key_id, issuer_id, key_path]):
        ...
        if creds_path.exists():
            for line in creds_path.read_text().splitlines():
                ...
                value = os.path.expandvars(os.path.expanduser(v.strip().strip('"').strip("'")))
...
def ensure_draft_version(client: ASCClient, app_id: str, preferred: str | None = None) -> dict:
    ...
    if preferred:
        ...
        candidate = bump_version(
            live["attributes"]["versionString"] if live else "1.0.0"
        )
    for _ in range(8):
...
def description_for_locale(locale: str, source: str = "en-US") -> str:
    ...
    if len(desc) < 10:
        desc = (
            ...
            or "DUPR IQ - pickleball shot-selection drills that grade the optimal hit."
        )
    return desc[:4000]
